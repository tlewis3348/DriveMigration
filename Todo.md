...
* [ ] **Dry Run Testing**: Run migration tool in the "dry run" mode to validate the map generation without making actual API calls. This allows you to verify the structure and metadata extraction logic.
  ...
  * [X] **Implement Immutable Target-ID Anchoring for Folder Shortcuts**:
    * [X] Update the directory traversal loop to intercept `application/vnd.google-apps.shortcut` objects targeting directories.
    * [X] Force the engine to write a predictable Markdown cross-reference link (`[Shortcut Name](#folder-targetId)`) immediately upon encounter, bypassing downstream traversal.
    * [X] Append a unique HTML anchor tag (`<a name="folder-currentId"></a>`) to every true folder list item written to `My_Life_and_Worldview.md` to serve as the invariant target destination for current and future cross-links.
